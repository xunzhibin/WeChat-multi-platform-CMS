...
// date_default_timezone_get() OR date_default_timezone_set('PRC');//设置中国时区
...
abstract class REST_Controller extends CI_Controller {
     ...
     */
    public function __construct($config = 'RESTful/rest')
    {
        parent::__construct();
	...
	//获取设置请求参数
	protected function get_setting_request_param()
	{
		//获取/设置 请求中必要的参数
        $_class = strtolower($this->router->class);//类名
        $_func = strtolower($this->router->method);//方法名

        $config_path = ucfirst($_class) . '/request';
        $config_info = ($this->load->config($config_path, FALSE, TRUE) && 
						$config_info = $this->config->item($this->request->method, $config_path))
						? $config_info : array();

        $config_request = '';
        if(array_key_exists($_func, $config_info))
        {
            $config_request_param = $config_info[$_func];
        }
        else if(array_key_exists($_class, $config_info))
        {
            $config_request_param = $config_info[$_class];
        }
        else
        {
            $config_request_param = array();
        }
        
        //请求参数
        $request_param = $this->{'_'.$this->request->method.'_args'};

		$this->request->param = $this->get_request_param($request_param, $config_request_param);
	}
	
	public function get_request_param($request_param, $config_request_param)
	{
		$result_row = array();
		$result_batch = array();

		//循环请求参数
		foreach($request_param as $request_key => $request_value)
		{
			//检索参数key 是否在配置中，并返回配置中key；否则跳出此次循环
			if(($search_key = array_search($request_key, $config_request_param)) === FALSE)
			{
				 continue;
			}
			...
			//检查 提交的请求值 是否为 数组转换的json字符串
			if(is_array($param = json_decode($request_value, TRUE)))
			{
				$result_batch = array_merge($result_batch, $param);
			}
			else
			{
				$result_row = array_merge($result_row, array($search_key => $request_value));
			}
		}
		...
		if($result_batch)
		{
			...
			{
				$new_row = $this->get_request_param($row, $config_request_param);

				$new_batch[] = array_merge($new_row, $result_row);
			}
